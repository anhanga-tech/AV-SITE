...
const BLOG_POST_ROUTE_PATTERN = /^\/blog\/([a-zA-Z0-9-]+)$/;
// Mesma gramática de slug aceita em lib/blog-mdx.ts (getBlogSlugFromUrl) e
// pages/BlogPost.tsx (isValidSlug): slugs com outros caracteres não geram
// rota de post e não devem exigir validação de corpo.
const BLOG_SLUG_PATTERN = /^[a-zA-Z0-9-]+$/;
...
export const validateHtml = (route, html) => {
  ...
  const blogPostMatch = normalizeRoute(route).match(BLOG_POST_ROUTE_PATTERN);
  if (blogPostMatch && BLOG_SLUG_PATTERN.test(blogPostMatch[1])) {
    const expectedMarker = `data-blog-post-body="${blogPostMatch[1]}"`;
    if (!html.includes(expectedMarker)) {
      throw new Error(`Missing complete blog post body in prerendered output for route "${route}"`);
    }
  }
};
