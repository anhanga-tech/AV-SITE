...
- `**/*.ts`, `**/*.tsx` outside `dist/`, `.prerender-ssr/`, `node_modules/`, and generated files (`data/blogManifest.ts`, `data/blogMarkdown.ts`, `data/blogManifestHomeTeaser.ts`)
...
- Generated files (`data/blogManifest.ts`, `data/blogMarkdown.ts`, `data/blogManifestHomeTeaser.ts`) are excluded entirely — they're machine-written on every build, matching the exception already documented in `docs/standards/code-style.md`.
