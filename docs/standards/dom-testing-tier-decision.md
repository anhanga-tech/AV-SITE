...
2. Primeira linha do arquivo: `import './helpers/dom-setup.ts';` — precisa rodar antes de qualquer `render()`, mas não precisa ser literalmente a primeira declaração de import (o setup só precisa terminar de avaliar antes da fase de execução dos testes, que já acontece depois de todos os imports do módulo). Por baixo, `dom-setup.ts` usa `@happy-dom/global-registrator` (pacote oficial do happy-dom) em vez de copiar `window`/`document`/`HTMLElement` manualmente — registra todos os construtores DOM (`Node`, `Event`, `CustomEvent` etc.) como globals de uma vez, então um teste futuro que precise de um construtor que os testes atuais não usam (ex.: `event.target instanceof Node`, comum em handlers de "clique fora") não quebra por faltar um global esquecido numa lista manual.
