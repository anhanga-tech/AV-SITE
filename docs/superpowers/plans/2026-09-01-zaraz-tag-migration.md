...
6. **P1 (chatgpt-codex-connector[bot]) — PENDENTE, sem correção nesta rodada.** O `anhanga_ga_cid` que geramos é um ID totalmente nosso, desconectado do client_id real que o Zaraz usa nos hits GA4. Confirmado lendo a fonte do Managed Component GA4 (`github.com/managed-components/google-analytics-4/src/requestBuilder.ts:101-107`): o Zaraz já gera e persiste seu próprio client_id via `client.set('ga4', cid, {scope:'infinite'})`, sem depender de nada que rodamos no browser. Pra um visitante sem cookie `_ga` legado, o `x_ga_client_id` gravado no CRM nunca vai bater com o client_id real das sessões GA4 desse visitante no Zaraz — a correlação CRM↔GA4 que a Task 296 tentou consertar continua quebrada nesse caso específico. **Correção correta:** ler o client_id que o Zaraz já persiste (provavelmente um cookie do próprio componente, algo como `zaraz_google-analytics_v4`, observado ao investigar o erro de `_ga_QDBT5PM4KP` na Task 1/2 — mas nunca confirmado byte a byte) em vez de inventar um novo. Não implementado porque exige inspecionar o cookie real no DevTools em produção — sem isso o parser seria um chute. **Ação para o usuário:** abrir DevTools no site em produção, achar o cookie `zaraz_google-analytics_v4` (ou nome equivalente da tool GA4) e compartilhar o formato do valor (JSON? string simples?) pra eu implementar a leitura correta.
