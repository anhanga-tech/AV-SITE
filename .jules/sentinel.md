...
## 2026-09-07 - [pnpm 11 Silently Ignores `pnpm.overrides` in package.json]
**Vulnerability:** `pnpm audit --prod` flagged `browserslist <= 4.28.6` (GHSA-c83g-rgw3-j3cx, GHSA-73wf-gq98-2v4g), pulled in transitively via `autoprefixer`. First fix attempt added the override under a `"pnpm": { "overrides": {...} }` key in `package.json` — the classic/documented location. `pnpm install` accepted it with **only a warning** (`The "pnpm" field in package.json is no longer read by pnpm... "pnpm.overrides" ignored`) and the lockfile stayed on the vulnerable version. No error, no install failure — the override was a silent no-op.
**Learning:** As of pnpm 10+ (this repo pins 11.3.0), the override/settings location moved to root `pnpm-workspace.yaml` under its own `overrides:` key (this repo already has 13+ documented CVE overrides there, one per advisory with GHSA IDs, transitive path, and version-choice rationale — I'd missed that convention on the first pass and nearly shipped a PR that "fixed" nothing while claiming green `pnpm audit`). Anyone following older pnpm docs/muscle memory (or an LLM trained on pre-v10 examples) will reach for `package.json` first and get a plausible-looking but inert result.
**Prevention:** Any dependency-override fix in this repo goes in `pnpm-workspace.yaml`'s `overrides:` block, never `package.json`. Always re-run `pnpm audit` (no `--prod`/`--dev` filter — this vulnerability's own two transitive paths span both trees, and the flags are mutually exclusive rather than additive) and `pnpm why <pkg>` *after* `pnpm install` to confirm the resolved version actually changed — a clean `pnpm install` exit code proves nothing about whether the override took effect, only that pnpm didn't error.
