...
Este sistema rejeita explicitamente: formulário corporativo frio, cinza neutro sem identidade, fonte sistema, espaçamento mecânico uniforme. Também recusa o SaaS genérico de gradiente azul-roxo, glassmorphism decorativo, e o template de métricas com número grande e fundo gradient.
...
- Poppins como voz da marca: geométrica, amigável, confiante sem ser agressiva
...
- Animações com `ease-out-quart` — nada de bounce ou elastic
...
- **Safira Noturna** (#003B8E): Par escuro da Safira Profunda. Hover em contextos de identidade, degrade fundos institucionais. OKLCH: `oklch(34% 0.17 264)`.
...
### Neutral
- **Ardósia Profunda** (#0f172a): Fundo principal em contextos dark. Também cor de texto dos botões primary e labels em contexto light. Tinted neutral — não é preto. OKLCH: `oklch(18% 0.012 264)`.
...
**Body Font:** Inter (com fallback `Poppins, sans-serif`)
...
**A Regra da Voz da Marca.** Inter é permitida apenas em body text e inputs. Headings, badges, labels e botões falam em Poppins — é a voz da Anhangá. Trocar para font do sistema nessas posições quebra a identidade.
...
### Shadow Vocabulary
- **hard** (`4px 4px 0px 0px #0f172a`): Botões primary e CTA em repouso sobre fundos claros. Sinal de "clicável com peso físico".
- **hard-yellow** (`4px 4px 0px 0px #FFD600`): Variante do hard para botão primary. O Âmbar Vivo como assinatura press.
- **hard-lg** (`8px 8px 0px 0px #0f172a`): Hard ampliado para elementos hero ou cards de destaque de grande formato.
- **hard-hover** (`6px 6px 0px 0px #0f172a`): Estado intermediário hover (entre rest e active). Nunca usado sozinho — parte da sequência animada.
...
- **Shape:** Varia por variante. Primary: arredondado médio (12px). Action: pílula completa (9999px). CTA: arredondado generoso (16px). Ghost: arredondado suave (8px).
- **Primary** (Ardósia Profunda, texto branco, `shadow-hard-yellow`): Para ações importantes em contexto light. Press-down com Âmbar Vivo. Tamanhos: `sm` (12px, px-4 py-2), `md` (14px, px-5 py-2.5), `lg` (16px, px-7 py-3.5).
...
- **Estados:** `disabled:opacity-50 disabled:cursor-not-allowed`. Foco: `outline-2 outline-offset-2 outline-action` em todos os variantes. Loading: ícone spin `CircleNotch` do `@phosphor-icons/react`.
...
- **Style:** Borda `2px border-gray-200`, bg branco, `rounded-xl` (12px), Poppins/Inter medium 14px, texto Ardósia.
...
### Navigation
*Chameleon confiante — adapta tom sem perder identidade.*
...
### Do:
- **Do** usar Poppins em todos os headings, badges, labels, e botões. Inter é permitida apenas em body text e campos de input.
...
### Don't:
- **Don't** usar formulário corporativo frio: cinza neutro, campos sem identidade, fonte sistema, grid mecânico uniforme. A Anhangá não é uma OTA genérica.
