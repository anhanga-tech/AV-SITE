...
5. **Evento `NEW_REVIEW_COMMENT` / reviews pendentes:** ler cada comentário (`.../pulls/N/comments` e `.../pulls/N/reviews`). Seguir a skill `receiving-code-review`: avaliar tecnicamente antes de aplicar — **nem todo apontamento do bot é válido**. Os dois bots já deram falso positivo aqui (reclamaram que arquivos não tinham sido atualizados quando já estavam corretos **no próprio commit analisado** — provável artefato de olhar hunks isolados do diff). Um comentário que se refere a um commit anterior ao revisado, ou que cita identificador que já não existe no arquivo, é o sinal típico. Antes de aceitar uma reclamação, conferir contra o conteúdo real: `git show <commit>:<arquivo>`.
