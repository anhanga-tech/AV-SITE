...
test('truncateProviderDetail returns a short detail unchanged', () => {
    assert.equal(truncateProviderDetail('boom'), 'boom');
});

test('truncateProviderDetail caps an oversized detail', () => {
    const huge = 'x'.repeat(50_000);
    const detail = truncateProviderDetail(huge);
