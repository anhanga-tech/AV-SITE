...
## 2026-07-22 - Optimizing Mobile Input Resets with Haptics and Focus Retention
**Learning:** On mobile devices, clicking a "Clear" button often causes the input to lose focus, which in turn dismisses the virtual keyboard. This creates a friction point if the user intended to clear the field to type a new value. Combining `triggerHaptic('light')` with explicit `inputRef.current?.focus()` ensures the keyboard remains active and provides a tactile confirmation of the reset action, significantly smoothing the search/correction flow.
**Action:** When implementing reset/clear buttons for mobile search inputs, always programmatically restore focus to the input and use light haptics to provide tactile confirmation.
